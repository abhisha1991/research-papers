...
The Hierarchical Navigable Small World (HNSW) graph has emerged as the dominant in-memory approximate nearest neighbor index for modern RAG systems. The key insight enabling HNSW's efficiency is hierarchical organization: rather than searching through all vectors linearly, HNSW structures vectors into multiple layers such that searching begins at a high level (where only a few vectors exist) and progressively descends, allowing the search to narrow down the location of the nearest neighbors. This hierarchical approach achieves near-logarithmic search complexity—similar to how binary search works on sorted arrays, but adapted for similarity search in high-dimensional spaces.

To understand HNSW intuitively, imagine searching for a city in a geographical hierarchy. At the top level, you identify the correct continent; then the country; then the state; finally the city. Each level narrows the search space exponentially. HNSW applies this principle to vector search. The structure comprises multiple layers indexed from 0 (the base layer containing all vectors) to L (the top layer containing very few vectors). Within each layer, vectors are connected as a graph where edges link similar vectors. The layer assignment follows an exponential decay: a vector is assigned to layer L with probability p = 0.5^L. This means if you pick a random vector, it is almost always assigned to the base layer (layer 0), occasionally to layer 1, rarely to layer 2, and extremely rarely to high layers. This probability distribution creates a natural pyramid: many vectors at the bottom, fewer in the middle, and very few at the top.

Search in HNSW leverages this pyramid structure for efficiency. To find the nearest neighbors of a query, the algorithm starts at the top layer (containing the fewest vectors) and performs a greedy search: it examines the few vectors at the top layer, identifies which is closest to the query, and jumps to that vector's neighbors. It repeats this process—jumping to the closest unvisited neighbor—until it cannot find a closer vector at the current layer. Once the search stagnates (no closer vector exists at the current layer), it descends to the next lower layer and repeats. By the time the search reaches the base layer, it has already identified a region of the vector space close to the query, dramatically reducing the work required to find the nearest neighbors. The total number of distance comparisons needed scales as O(log N), making search both fast and scalable.

The insertion process is analogous to building this hierarchical structure incrementally. When a new vector arrives, the algorithm first randomly assigns it a maximum layer, respecting the exponential decay probabilities. Then it inserts the vector into the graph at each layer from the top down to its assigned maximum layer. For each layer, the algorithm identifies candidate neighbors (other vectors that might be good neighbors for the new vector) and selects up to M of them using a connectivity heuristic. The heuristic is designed to maintain graph connectivity: it preferentially keeps neighbors that are close to the new vector while discarding candidates where another already-selected neighbor is much nearer. This ensures the graph remains densely connected without becoming overly dense, which would slow searches. After selecting neighbors, the algorithm creates bidirectional edges: the new vector links to its selected neighbors, and those neighbors link back to the new vector.

Three parameters control HNSW behavior and determine the balance between accuracy, speed, and memory. The parameter M controls the maximum number of edges per vector (the out-degree in the graph). Increasing M makes the graph denser, enabling neighbors to find each other more easily and improving recall (the fraction of true nearest neighbors found), but it also increases memory usage and slows both insertion and search. The parameter ef_construction is a beam width used during insertion: when selecting neighbors, the algorithm searches for candidates with a broad beam to find better neighbors, but a wider beam requires examining more candidates and thus slows insertion. Finally, ef is a query-time parameter that controls how thoroughly the search explores the graph: larger ef values mean more thorough exploration and better accuracy, but slower queries. In practice, configurations use M ∈ [8, 64], ef_construction ∈ [100, 400], and ef ∈ [100, 200], with the exact values tuned for the application's latency and accuracy requirements.

Deletion introduces challenges. HNSW does not physically remove deleted vectors from the graph; instead, it marks them as deleted and continues using them for path navigation during searches. When a vector is deleted, searches can still traverse through it to reach other vectors, but deleted vectors are never returned as results. This lazy deletion approach simplifies implementation: there is no need to reconstruct large portions of the graph when a vector is deleted.

However, updating a deleted vector (inserting a new vector with the same ID) requires careful handling. The replaced_update algorithm attempts to repair the graph by reselecting neighbors for vectors that were adjacent to the deleted vector. However, recent research by Xiao et al. (2024) identified a subtle but important problem: this reselection process can inadvertently create unreachable points—vectors that exist in the graph but cannot be discovered through greedy search starting from any entry point.

To understand why unreachable points arise, consider a concrete example. Suppose vector D has outgoing edges to neighbors A, B, and C, and vector V is a distant neighbor that was only reachable by first reaching D and then following an edge from D to A, and then from A to V. When D is deleted and replaced with a new vector D', the reselection algorithm updates A, B, and C's incoming edges by finding new neighbors for these vectors. However, if the algorithm doesn't carefully consider all indirect neighbors (vectors at distance 2 or more), V may end up with no incoming edges from any vector at any layer. Since greedy search always moves to the closest neighbor, it will never reach V, making it permanently unreachable.

This phenomenon is particularly problematic because recall degrades silently as unreachable points accumulate. In an HNSW index experiencing continuous insertions and deletions, the fraction of unreachable points grows by approximately 3-4% per 3000 insert-delete cycles, translating to a 1-3% recall loss per 1000 cycles. For systems with high update rates, this degradation quickly becomes unacceptable.

The MN-RU (Mutual Neighbor Replaced Update) algorithm provides a practical solution. Rather than considering all neighbors of the deleted vector, MN-RU restricts reselection to mutual neighbors—vectors that had bidirectional edges with the deleted vector. This constraint is conservative: it updates fewer neighbors and is more cautious about changing the graph structure. In exchange, the algorithm prevents most unreachable points from forming. In practice, MN-RU limits unreachable point growth to less than 1% per 3000 iterations and simultaneously reduces update complexity from O(M³) to O(M²) per layer, achieving a 2-4x speedup for update operations. For systems requiring dynamic indexing, MN-RU makes HNSW indices practical for real-time workloads.

An alternative mitigation strategy is to maintain a secondary index: a separate backup HNSW index that is periodically rebuilt from the main index, extracting orphaned and unreachable vectors. At query time, the system searches both the main and backup indices, merging the results. This approach guarantees no recall loss—all vectors are discoverable through one index or the other—at the cost of additional storage (maintaining two indices) and query latency (searching two indices and merging results).
...
Before discussing specific quantization techniques, it is helpful to understand the fundamental tradeoff. Quantization is a data compression technique that converts high-precision values into lower-precision representations. For embeddings, this means converting 32-bit floats (which can represent any real number with high precision) into smaller representations such as 8-bit integers. The compression reduces memory dramatically, but loses information: two distinct float values might map to the same 8-bit integer, introducing quantization error. The art of quantization is choosing the right level of precision loss—aggressive enough to achieve meaningful compression, conservative enough to preserve search quality.

Scalar Quantization (SQ) is the simplest quantization approach: it quantizes each dimension independently using uniform quantization. The simplest variant, SQ-8, maps float32 values in the range [-1, 1] to unsigned 8-bit integers via:

```
quantized = round((value + 1) / 2 × 255)
```

This transformation maps -1 to 0, 0 to 127.5 (rounded to 128), and 1 to 255. The reverse transformation reconstructs approximate float values. This approach is lossy—precision is lost—but offers several advantages. First, the mapping is deterministic and requires no training: any float value is directly converted without consulting a lookup table. Second, distance computations proceed directly on quantized values using 8-bit arithmetic, which is much faster than float arithmetic on modern CPUs. Third, the memory reduction is straightforward: 4 bytes per dimension becomes 1 byte per dimension, yielding 4x compression.

In practice, recall degrades by 2-5% depending on the distribution of original embedding values and the distribution of queries. SQ-8 is suitable for applications where a small recall loss is acceptable. The simplicity and speed of SQ-8 make it a popular choice for systems with memory constraints. SQ-4 and SQ-2 (4-bit and 2-bit variants) achieve even more aggressive compression (8x and 16x respectively) but at the cost of higher recall loss (10-20%).

Product Quantization (PQ) is more sophisticated. Rather than quantizing all dimensions together, PQ partitions vectors into disjoint segments and quantizes each segment independently using a learned codebook. To illustrate, consider a 768-dimensional vector partitioned into eight 96-dimensional segments. For each segment, PQ trains a k-means codebook containing 256 cluster centroids. When a new vector arrives, each of its eight segments is compared against the 256 centroids, and the index of the closest centroid is recorded as a single 8-bit value. The entire vector is thus encoded in 64 bits—only 8 values × 8 bits each—a 48x compression ratio compared to the original 3072 bits (768 dimensions × 4 bytes).

The codebook is trained offline on a representative sample of the corpus using k-means clustering. For instance, if training on one million vectors, the algorithm would randomly select sample segments from these vectors and cluster them into 256 groups. When new vectors arrive after training, they are immediately quantized using the existing codebooks. Periodically, codebooks are retrained on accumulated new data to improve alignment with the evolved distribution. This retraining is a background operation that does not block ongoing queries. Once retraining completes, the system begins using the new codebook for all subsequent embeddings; old embeddings remain quantized with the old codebook until the index is fully refreshed during a scheduled maintenance window. This deferred transition avoids the need for bulk re-quantization.

The trade-off between compression and accuracy is significant in PQ. With eight segments, each with 256 codewords, each segment is quantized to its nearest centroid. This introduces discretization error: all vectors whose 96-dimensional segment falls within a cluster get mapped to the same centroid. Larger segment sizes reduce codebook bias (since centroids represent more diverse data) but increase quantization error (since segments are less granular). Smaller segment sizes improve granularity but require more codebook training to avoid underfitting. In practice, PQ with 8 segments typically loses 5-15% recall compared to full-precision search, depending on segment size, codebook quality, and query distribution.

Other quantization techniques exist along the complexity-compression tradeoff spectrum. Binary Quantization (BQ) quantizes each dimension to a single bit (1 or 0), achieving extreme compression—16-32x—but at the cost of significant accuracy loss (20-40% recall reduction). BQ is useful only for applications where rough similarity retrieval is sufficient as a first-stage filter before more expensive second-stage ranking. Rotational Quantization (RQ) applies learned rotations to vectors before quantization, attempting to align the vector space with coordinate axes to maximize compression without accuracy loss. RQ achieves 16x compression with 10-15% recall loss, offering a middle ground between SQ and PQ.

The choice of quantization technique depends on system requirements. If the application demands high accuracy (>95% recall), SQ-8 is often sufficient and preferred due to simplicity. If moderate accuracy is acceptable (90% recall) and memory is constrained, PQ offers superior compression. If recall below 90% is tolerable, BQ provides extreme compression for first-stage filtering. In production systems, it is common to employ hierarchical approaches: use PQ for the main index to save memory, and maintain a small high-precision backup index for critical queries. Alternatively, use different quantization schemes for different data tiers (e.g., full-precision for hot data, PQ for warm data, BQ for cold archival data).
